...
**DeepSeek reasoning runs at effort `low`.** It was disabled on the theory that these prompts hand the model every fact it needs and only ask for prose. That was wrong: with thinking off, recaps were reliably fluent and reliably bad at arithmetic over the tables in their own prompt — a 22-point gap written as five, a 98-point total read as a 98-point lead, an 11th called a driver's first scoreless race in a season that already held a 20th. Effort `low` cleared those. It costs 80–170s per article instead of ~23s, and `max_tokens` covers the reasoning trace *and* the output, so `_deepseek_json` adds `DEEPSEEK_REASONING_HEADROOM` to whatever the caller asked for; a truncated response still raises naming the cause. Set `DEEPSEEK_REASONING_EFFORT=none` for a bulk regeneration where speed matters more than a wrong number, or `high`/`max` to spend more.
